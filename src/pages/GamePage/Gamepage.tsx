...
function Gamepage() {
  ...
  const OPERATOR_TILES = TILES.filter(
    (tile) => tile.bgColor === TILES_COLORS.OPERATOR
  );
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const isMovedFromRackToBoard =
      tileLocations[active.id] === "pool" &&
      String(over.id).startsWith("square");
  ...
  return (
    <DndContext onDragEnd={handleDragEnd} sensors={sensors}>
      <div
        className="
  flex 
  flex-col-reverse    // En mobile: primero Board, luego Rack
  md:flex-row         // En desktop: en fila
  items-center 
  justify-center 
  min-h-screen 
  bg-gray-900 
...
"
      >
        ...
        <PlayerRack player={player}>
          ...
          <button onClick={() => updatePlayer({ score: player.score + 1 })}>
            Click Me
          </button>
        </PlayerRack>
