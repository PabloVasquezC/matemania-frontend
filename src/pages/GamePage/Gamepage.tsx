...
const MessageBox = ({
  ...
  const baseClasses =
    "fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 p-6 md:p-8 rounded-2xl shadow-2xl transition-all duration-300 max-w-sm w-11/12 text-center font-bold text-white transform animate-scale-in backdrop-blur-md";
  let colorClasses = "";

  switch (message.type) {
    case "success":
      colorClasses =
        "bg-gradient-to-br from-green-600/95 to-teal-600/95 border-4 border-green-400 shadow-green-500/50";
      break;
    case "error": // Usamos rojo para el Game Over
      colorClasses =
        "bg-gradient-to-br from-red-600/95 to-rose-600/95 border-4 border-red-400 shadow-red-500/50";
      break;
    case "info":
      colorClasses =
        "bg-gradient-to-br from-blue-600/95 to-indigo-600/95 border-4 border-blue-400 shadow-blue-500/50";
      break;
  }
  ...
  return (
    <>
      ...
      <div className={`${baseClasses} ${colorClasses}`}>
        <p className="text-lg md:text-xl mb-4 md:mb-6 leading-relaxed whitespace-pre-line">
          {message.text}
        </p>
        <button
          onClick={onClose}
...
function Gamepage() {
  ...
  const showMessage = useCallback(
    (text: string, type: MessageState["type"] = "info") => {
  ...
  useEffect(() => {
    ...
    const steps: StepOptions[] = [
      ...
      },
      {
        ...
        buttons: [
          { text: "Anterior", action: tour.back },
          { text: "¡A Jugar!", action: tour.complete },
        ],
      },
    ];
  ...
  useEffect(() => {
    ...
    const initialTiles = generateRandomTiles(100, gameMode);
  ...
  useEffect(() => {
    ...
    if (isGameOver || timeLeft <= 0) {
      if (timeLeft === 0 && !isGameOver) {
        ...
        showMessage(
          `⏰ ¡Tiempo agotado! \n Tu puntuación final es: ${player.score}`,
          "error"
        );
      }
      return;
